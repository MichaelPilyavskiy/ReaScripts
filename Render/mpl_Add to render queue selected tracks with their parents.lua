-- @description Add to render queue selected tracks with their parents
-- @version 1.01
-- @author MPL
-- @website http://forum.cockos.com/showthread.php?t=188335
-- @changelog
--    # fix check for parent track
  ...
  function main()
    ...
    for i = 1, #GUID_t do
      ...
      if GUID_t[i].is_sel ==1 then
          ...
          local par_track = GetParentTrack(src_tr)
          if par_track then 
            SetMediaTrackInfo_Value(send_track, "I_SELECTED", 1)
            SetMediaTrackInfo_Value(send_track, "B_MUTE", 0)
            SetMediaTrackInfo_Value(send_track, "I_SOLO", 0)
            UpdateArrange()
