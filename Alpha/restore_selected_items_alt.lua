retval, count_stored_guids_s = reaper.GetExtState("Sel_items_guid", "count")
...
if count_stored_guids ~= nil  then
 if count_stored_guids > 0 then 
  for i = 1, count_stored_guids do
    retval, stored_guid = reaper.GetExtState("Sel_items_guid", i)
    reaper.DeleteExtState("Sel_items_guid", i, true)
    retval, stored_track_guid = reaper.GetExtState("Sel_items_tracks_guid", i)
