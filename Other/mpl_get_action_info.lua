    ...
    for i=1, #kb_shortcuts_t do
      ...
      if string.find(kb_shortcuts_item, command_ID)~= nil and string.find(kb_shortcuts_item, "KEY") == nil then
        ...
        if string.find(kb_shortcuts_item, "SCR") ~= nil then
          ...
          script_location = string.sub(kb_shortcuts_item, kb_shortcuts_item_t[4]+3, -1)
...
---------------------------------------------------------------
   ...
   function search_langpack_table(action0)
          
    if action0 ~= nil and action0 ~= "" then
     st_val_t2 = {}
     st = 0
     for i = 1, 4 do
       st = string.find(action0, ' ', st+1)
       table.insert(st_val_t2, st)
     end
     
     if string.find(action0, "Main") == 1 then
       action1 = string.sub(action0, st_val_t2[3]+2)
     end
     
     if string.find(action0, "Media Explorer") == 1 then
       action1 = string.sub(action0, st_val_t2[3]+8)
     end
     
     if string.find(action0, "MIDI Editor") == 1 then
       action1 = string.sub(action0, st_val_t2[3]+8)
       test = action1
     end
     
     for i = 1, #langpack_table do
       langpack_line = langpack_table[i]
       st_lang = string.find(langpack_line, action1)
       if st_lang ~= nil then translated_action0 = langpack_line end
     end     
    end 
    
    
    
    ret_langpack_loc = get_Lp_location()
    if ret_langpack_loc ~= nil then 
    pack_file_t = Get_table_from_file(ret_langpack_loc)
    end
    
    
    
    if translated_action0 == nil then       translated_action0 = ""          end
    
   --[[ st_5 = string.find(translated_action0, "=")
    if st_5~= nil then  translated_act_id = string.sub(translated_action0,0,st_5-1) end
    if translated_act_id ~= nil and pack_file_t ~= nil then  
      for i=1, #pack_file_t do
        pack_item = pack_file_t[i]
        st_6 = string.find(pack_item, translated_act_id)
        if st_6 ~= nil then translated_action0 = string.sub(pack_item, string.len(translated_act_id)) break end
      end
    end]]
    
--[[    return translated_action0
   end  
