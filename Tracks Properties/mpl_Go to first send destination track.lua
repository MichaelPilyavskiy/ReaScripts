-- @description Go to first send destination track
-- @version 1.01
-- @author MPL
...
--  # remove SWS dependency
  ...
  function main()
    ...
    local receive_tr = GetTrackSendInfo_Value( tr, 0, send_id-1, 'P_DESTTRACK' )
  ...
  local ret = VF_CheckFunctions(3.60) if ret then local ret2 = VF_CheckReaperVrs(6.78,true) if ret2 then 
    Undo_BeginBlock2( 0 )
    main() 
    Undo_EndBlock2( 0, "Go to first send destination track", 0xFFFFFFFF )
  end end