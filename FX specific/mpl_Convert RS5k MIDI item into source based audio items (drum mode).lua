  ...
  function ConvertRS5kMIDI2AudioItems(item)
    ...
    -- build pitch map
      ...
      for fx = 1, TrackFX_GetCount( tr ) do
        ...
        if retval and buf == 'Note range start' then
          ...
          local retval2, sample_path = reaper.TrackFX_GetNamedConfigParm( tr, fx-1, 'FILE0' )
          if retval2 and sample_path then 
    ...
    -- collect data
      ...
      for i = 1,  ({MIDI_CountEvts( take )})[2] do 
        ...
        local pos = MIDI_GetProjTimeFromPPQPos( take, sppq )
        data[#data+1] = {mute = m,
                         pos =  pos,
                         len =  MIDI_GetProjTimeFromPPQPos( take, eppq )-pos,
                         vol = v/127,
                         pitch=p}
      end
    ...
    -- add audio items
      ...
      for i = 1, #data do
        ...
        if p_map[p0] then
          ...
          SetMediaItemInfo_Value( new_item, 'D_LENGTH', data[i].len)--(p_map[p0].end_offs - p_map[p0].start_offs)  * p_map[p0].srclen/prate   ) 
