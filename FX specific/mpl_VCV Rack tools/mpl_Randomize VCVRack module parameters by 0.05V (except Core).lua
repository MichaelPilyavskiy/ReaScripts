-- @description Randomize VCVRack module parameters by 0.05V (except Core)
-- @version 1.0
-- @author MPL
    ...
    for moduleID =1, #t.modules do
      for param_ID =1, #t.modules[moduleID].params do
        if not t.modules[moduleID].plugin:match('Core') then  
          t.modules[moduleID].params[param_ID].value = t.modules[moduleID].params[param_ID].value + math.random()*0.1-0.05
        end
      end
    end
  end
