...
-- @changelog
--    + link bandwidth after building
--    + bands gain reduce factor
--    + remember settings
  ...
  -----------------------------------------------------------------------------
  function MPL_BuildEQReduce(freq, gain, slope, linkBW)
...
[[      <PROGRAMENV ]]..(param+1)..[[ 0
    ...
    if linkBW then 
      local out_chunk = LinkBands(tr, fx, build_str)
  ...
  -----------------------------------------------------------------------------
  function LinkBands(tr, fx, build_str)
  ...
  ----------------------------------------------------------------------------- 
  local def_params = '50,-150,0.01,Y'
  local extstate = GetExtState('MPL_BUILDHARMSERREAEQ', 'params')
  if extstate ~= '' then def_params = extstate end
  local retval, str = GetUserInputs('Build harmonic bands', 4, 'Base frequency(Hz) (def=50),Gain Reduction(dB) (def=-150),Gain reduce (def=0.01),Link banwidth (Y/N)', def_params)
  if retval then 
    local t, freq, gain, slope, linkbands = {}, _, 0, 0.01, true
    ...
    MPL_BuildEQReduce(freq, lim(gain, 0, 0.5), slope, linkbands)
  end