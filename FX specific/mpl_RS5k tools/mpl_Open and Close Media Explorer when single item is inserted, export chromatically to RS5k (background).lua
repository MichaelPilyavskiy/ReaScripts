...
--    # use VF version check
  ...
  function ExportSelItemsToRs5k(track)
    ...
    if not item then return end
      ...
      reaper.TrackFX_SetParamNormalized( track, rs5k_pos, 5, 0.2 ) -- pitch for start
      ...
      reaper.TrackFX_SetNamedConfigParm(track, rs5k_pos, "MODE", 2)
    ...
    if count_items0 ~= count_items or isME_open() then 
      if not isME_open() then reaper.Main_OnCommand(50124, 0)  end
        ...
        if f_item then 
          ...
          ExportSelItemsToRs5k(f_item_tr)
