-- @description Clear spectral edits from selected items active take
-- @version 1.0
-- @author MPL
...
--------------------------------------------------------------------
  function main() 
    for i =1, CountSelectedMediaItems( -1 ) do
      ...
      MPL_SpectralEdits_Manipulate(activetake, {clear = true})  
...
--------------------------------------------------------------------  
  if VF_CheckReaperVrs(7.31,true) then 
    ...
    main()
    reaper.Undo_EndBlock("Clear spectral edits from selected items active take", 0xFFFFFFFF)
  end   