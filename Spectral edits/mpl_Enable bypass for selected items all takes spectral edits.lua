-- @description Enable bypass for selected items all takes spectral edits
-- @version 1.0
-- @author MPL
...
--------------------------------------------------------------------
  function main() 
    for i =1, CountSelectedMediaItems( -1 ) do
      ...
      MPL_SpectralEdits_Manipulate(activetake, {set_bypass = 0})  
...
--------------------------------------------------------------------  
  if VF_CheckReaperVrs(7.31,true) then 
    ...
    main()
    reaper.Undo_EndBlock("Enable bypass for selected items all takes spectral edits", 0xFFFFFFFF)
  end   