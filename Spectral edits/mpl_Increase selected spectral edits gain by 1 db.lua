-- @description Increase selected spectral edits gain by 1 db
-- @version 1.0
-- @author MPL
...
--------------------------------------------------------------------
  function main() 
    for i =1, CountSelectedMediaItems( -1 ) do
      ...
      MPL_SpectralEdits_Manipulate(activetake, {GAIN_DB_CHANGE = 1, APP_SEL_ONLY = true})  
...
--------------------------------------------------------------------  
  if VF_CheckReaperVrs(7.31,true) then 
    ...
    main()
    reaper.Undo_EndBlock("Increase selected spectral edits gain by 1 db", 0xFFFFFFFF)
  end   