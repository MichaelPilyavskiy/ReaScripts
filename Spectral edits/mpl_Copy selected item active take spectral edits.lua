...
--------------------------------------------------------------------
  function main()
    item = reaper.GetSelectedMediaItem(-1,0)
    if not item then return end 
    local take = GetActiveTake(item)
    if not (take and TakeIsMIDI(take)~= true) then return end 
    local t = MPL_SpectralEdits_Manipulate(take)
    outs = table.save_tostring(t )
    local outstr = encBase64(outs)
    SetExtState( 'MPL_SEtools', 'buf', outstr, false )
  end
--------------------------------------------------------------------  
  if VF_CheckReaperVrs(7.31,true) then 
    --reaper.Undo_BeginBlock() 
    main()
    --reaper.Undo_EndBlock(Copy selected item active take spectral edits, 0xFFFFFFFF)
  end   