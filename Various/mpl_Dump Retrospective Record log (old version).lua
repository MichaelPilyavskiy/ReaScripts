-- @description Dump Retrospective Record log (old version)
-- @version 1.01
-- @author MPL, Justin
...
--    # fix description, remove VF dependency
  ...
  if tonumber(reaper.GetAppVersion():match('[%d%.]+')) >= 6.39 then DumpRetrospectiveLog() else reaper.MB('This script require REAPER 6.39+','MPL Dump retrospective record',0) end
