...
--    + init
  ...
  ---------------------------------------------------
  function TempoFit(t0,t1,allow_partial_measure, tempo0, timesigdenum0)  
    local tempo0_onebeattime = 60/tempo0
    local timediff = t1-t0
    beats_diff= math_q(timediff/tempo0_onebeattime)   
    if allow_partial_measure then
      timesigdenum = beats_diff
     else
      beats_diff = math_q(beats_diff/timesigdenum0 )*timesigdenum0  
    end    
    ...
    AddTempoTimeSigMarker( 0, t0, tempo, timesigdenum, 4, false )    
  end
  ---------------------------------------------------  
  function AutoWarpGrid()
    ...
    if t and #t > 2 then
      ...
      retUI, str = GetUserInputs(scr_title,3,'Desired tempo,Time signature numerator,Allow partial measure (Y/N)',cur_tempo..',4,Y')
      if retUI then
        val_t = {}
        for val in str:gmatch('[^,]+') do val_t[#val_t+1] = val end
        if #t < 3 or not tonumber(val_t[1]) or not tonumber(val_t[2]) then return end
        local desired_tempo = tonumber(val_t[1])
        local timesig_num = tonumber(val_t[2])
         allow_partial_measure = val_t[3]:lower():match('y')
        Undo_BeginBlock()
        SetMediaItemInfo_Value( item, 'C_BEATATTACHMODE', 2 )
        ClearTimeSignatureMarkers()
        for i = 1, #t-1 do TempoFit(t[i], t[i+1], allow_partial_measure, desired_tempo, timesig_num) end
