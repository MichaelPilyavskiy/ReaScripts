-- @description Remove selected takes MIDI CC64 (hold pedal)
-- @about
...
--    + init
  ...
  local scr_title = 'Remove selected takes MIDI CC64 (hold pedal)'
  ...
  function FilterMIDIData(take, exclude_msg_type)
    ...
    while stringPos < MIDIlen-12 do -- -12 to exclude final All-Notes-Off message
      ...
      if msg:len() > 1 then
        if msg:byte(1)>>4 == exclude_msg_type and msg:byte(2) == 64 then
