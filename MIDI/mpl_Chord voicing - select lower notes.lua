-- @description Chord voicing - select lower notes
-- @version 1.0
-- @author MPL
...
--  + init
    ...
    for ppq in spairs(chords) do
      ...
      if has_selectedflag then 
        for chordnote = 1, #chords[ppq] do 
          ...
          if chordnote == 1 then chords[ppq][chordnote].flags = 1 else chords[ppq][chordnote].flags = 0 end
        end 
      end
    end
  end
  ...
  local ret = VF_CheckFunctions(3.51) if ret then local ret2 = VF_CheckReaperVrs(5.32,true) if ret2 then 
    ...
    Undo_EndBlock('Chord voicing - select lower notes', 0xFFFFFFFF)
  end end