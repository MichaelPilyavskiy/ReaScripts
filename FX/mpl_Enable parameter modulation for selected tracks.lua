-- @description Enable parameter modulation for selected tracks
-- @version 1.01
-- @author MPL
...
--    # VF independent
  ...
  script_title_out = 'Enable parameter modulation for selected tracks'
  ---------------------------------------------------
    ...
    for i = 1, CountSelectedTracks()do
      ...
      for fxnumber = 1,  TrackFX_GetCount( tr ) do
        for paramnumber = 1,  TrackFX_GetNumParams( tr, fxnumber-1 ) do
          ...
          if ret then TrackFX_SetNamedConfigParm( tr, fxnumber-1,  'param.'..(paramnumber-1)..'.mod.active',1 ) end  
        end
      end
    end
  end
