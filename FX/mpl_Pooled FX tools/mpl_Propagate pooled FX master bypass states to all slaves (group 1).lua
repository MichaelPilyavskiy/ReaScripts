-- @description Propagate pooled FX master bypass states to all slaves (group 1)
-- @version 1.0
-- @author MPL
...
--    + init
 ...
 ---------------------------------------------------------------------  
  function PropagatePoolFX_state(dest_track, src_track, jsfx_id, fx_data)
    ...
    for i = 1, #fx_data do
      local parmcnt = TrackFX_GetNumParams(  dest_track, i+jsfx_id)
      TrackFX_SetParam( dest_track, i+jsfx_id, parmcnt-2,fx_data[i].params[parmcnt-1]) 
    end
  end
 ---------------------------------------------------------------------
  ...
  if ret then
    ...
    if ret and ret2 then  
      Undo_BeginBlock()
      main(groupID)
      Undo_EndBlock( 'Propagate pooled FX master bypass state to all slaves (group 1)', -1 )
    end
  end
