-- @description Add pooled fx slave JSFX to selected tracks at chain begin
-- @version 1.0
-- @author MPL
...
--    + init
    ...
    for i = 1, CountSelectedTracks(0) do
      ...
      if not tr_name:match('POOL FX%d+ master') then
        ...
        if ret < 0 then 
          ...
          fxcnt = TrackFX_GetCount( track )
          TrackFX_CopyToTrack( track, fxcnt-1, track, 0, true )
        end
  ...
  if ret then
    ...
    if ret and ret2 then 
      Undo_BeginBlock() 
      main()
      Undo_EndBlock('Add pooled fx slave JSFX to selected tracks at chain begin', -1) 
    end
  end
