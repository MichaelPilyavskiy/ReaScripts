-- @description Bypass all the selected track FX starting from focused FX
-- @version 1.0
-- @author MPL
...
--    + init 
  ...
  function main()
    ...
    for fx = fxnumber,  TrackFX_GetCount( track )-1 do
  ...
  local ret = VF_CheckFunctions(2.84) if ret then local ret2 = VF_CheckReaperVrs(5.975,true) if ret2 then 
    Undo_BeginBlock2( 0 )
    main() 
    Undo_EndBlock2( 0, 'Bypass all the selected track FX starting from focused FX', 0 )
  end end